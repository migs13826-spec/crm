...
function getPasswordStrength(password: string): { level: string; color: string; width: string } {
  ...
  if (score <= 3) return { level: "Fair", color: "bg-amber-500", width: "66%" };
...
export default function SignupPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <div className="rounded-xl bg-red-50 p-3.5 text-sm text-red-600 border border-red-100">
          {error}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-start gap-2.5 pt-1">
          ...
          />
          <Label htmlFor="terms" className="text-xs text-gray-500 font-normal leading-5 cursor-pointer">
            I agree to the{" "}
